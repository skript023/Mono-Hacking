...
namespace big
{
	namespace
	{
		...
		{
			panel_frame = ImGui::GetFrameCount(); panel_section = section;
			base_tools::request_scan();
		}
		void queue(void (*action)()) { g_fiber_pool->queue_job(action); }
		void refresh(regular_submenu* sub)
		{
			sub->add_option<reguler_option>("Refresh Nearby Objects", "Scan loaded objects within the selected radius.", [] { base_tools::request_scan(); });
		}
	}
	...
	void view::base_tools_submenu()
	{
		...
		canvas::add_submenu<regular_submenu>("Quick Stack", "BaseStack"_hash, [](regular_submenu* sub) {
			...
			sub->add_option<reguler_option>("Stack into Nearby Chests", "Only matching item types; equipped items remain with you.", [] { base_tools::set_options(config); queue(base_tools::quick_stack); });
		});
		canvas::add_submenu<regular_submenu>("Production Monitor", "BaseProduction"_hash, [](regular_submenu* sub) { show_panel(0); refresh(sub); });
		canvas::add_submenu<regular_submenu>("Repair Radius", "BaseRepair"_hash, [](regular_submenu* sub) {
			...
			sub->add_option<reguler_option>("Repair Nearby Buildings", "Send repair requests for accessible damaged pieces.", [] { queue(base_tools::repair_nearby); });
			refresh(sub);
		});
		canvas::add_submenu<regular_submenu>("Farming Assistant", "BaseFarming"_hash, [](regular_submenu* sub) {
			...
			sub->add_option<reguler_option>("Grow Healthy Plants Now", "Only healthy, accessible plants owned by this client.", [] { queue(base_tools::grow_nearby); });
			refresh(sub);
		});
		canvas::add_submenu<regular_submenu>("Production Speed", "BaseSpeed"_hash, [](regular_submenu* sub) {
			...
			sub->add_option<reguler_option>("Reset All Speeds to 1x", "Restore normal production and plant growth speeds.", [] { config.smelter_speed = config.fermenter_speed = config.honey_speed = config.plant_speed = 1.f; });
		});
		canvas::add_submenu<regular_submenu>("Weather & Daylight", "BaseEnvironment"_hash, [](regular_submenu* sub) {
			...
			sub->add_option<reguler_option>("Restore Automatic Environment", "Clear weather and daylight overrides.", [] { config.weather.clear(); config.lock_daylight = false; });
		});
		canvas::add_submenu<regular_submenu>("Select Weather", "BaseWeather"_hash, [](regular_submenu* sub) {
			base_tools::request_scan();
			sub->add_option<reguler_option>("Automatic Weather", "Use the game's normal environment selection.", [] { config.weather.clear(); });
			for (const auto& weather : base_tools::get_snapshot().weather)
				sub->add_option<reguler_option>((weather + (config.weather == weather ? " (Selected)" : "")).c_str(), "Apply this environment locally.", [weather] { config.weather = weather; });
			refresh(sub);
		});
		canvas::add_submenu<regular_submenu>("Comfort Inspector", "BaseComfort"_hash, [](regular_submenu* sub) { show_panel(3); refresh(sub); });
	}
	...
	void view::base_tools_panel()
	{
		...
		if (ImGui::Begin("Base Inspector", nullptr, ImGuiWindowFlags_NoCollapse))
		{
			...
			ImGui::Separator();
			if (!state.ready) ImGui::TextUnformatted("Waiting for nearby objects. Join a world to scan your base.");
			else
			{
				if (panel_section == 3)
				{
				...
				const auto& rows = panel_section == 0 ? state.production : panel_section == 1 ? state.buildings : panel_section == 2 ? state.plants : state.comfort;
				ImGui::Text("%zu objects | Updates every second", rows.size());
				if (rows.empty()) ImGui::TextUnformatted("No matching loaded objects in range.");
				for (const auto& row : rows)
				{
					ImGui::Separator(); ImGui::TextUnformatted(row.name.c_str()); ImGui::TextUnformatted(row.detail.c_str());
